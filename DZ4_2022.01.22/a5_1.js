...
console.log(`Сумма отрицательных элементов массива - ${sum_otr.toFixed(2)}`);
/*-------------------------*/
sum_between_nuls = 0;
let indexFirstNull;
let countIndexNull = 0;
arr.forEach((item, index) => {
    if (item == 0){
        countIndexNull++;  
        indexFirstNull = index;                      
    } 
    if (countIndexNull < 2 && index >= indexFirstNull)  {
        sum_between_nuls +=item;
        
    } 
})
console.log(`Сумма элементов массива между двумя первыми нулями - ${sum_between_nuls.toFixed(2)}`)
/*--------------------*/
let min_chet = arr[0];
arr.forEach((item) => {
    if(item%2 == 0 && item < min_chet){
        item = min_chet;
    }
})
console.log(`Наименьший четный элемент массива - ${min_chet}`);
/*------------------*/
let min = arr[0];
let index_min;
let max = arr[0];
...
console.log(`Сумма номеров минимального и максимального элементов-${index_max + index_min}` );
/*-----------------------*/
//массив в котором будут только положительные числа исходного массива
let new_arr = [];
arr.map((item) => {
    if (item > 0){
        new_arr.push(item);
...
console.log(`массив в котором будут только положительные числа исходного массива - ${new_arr}`);
/*-------------------------*/
//Округлите все числа массива
//Oкруглите все числа до целого вверх
new_arr = [];
new_arr2 = [];
arr.map((item) => {
    new_arr.push(Number(item.toFixed(2)));
    new_arr.push(Math.ceil(item));
})
console.log(`Округленные числа`);
console.log(new_arr);
